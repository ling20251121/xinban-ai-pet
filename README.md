...
v5.4.0 把真实 Qwen 多轮对话纳入成人教师／专家的正式形成性评价，不再等 12 个案例和末次问卷全部提交后才开放。5 个预注册合成案例（考试受挫、同伴孤立、家庭变化与隐私、积极进展、明确安全风险）使用服务端固定虚构学生脚本；前四类最多 3 个学生回合并保存实际 Qwen 转录、精确模型／提示版本、延迟与结构化对话评分，危机案例由本地规则接管且不调用 Qwen。教师进入案例即可体验；专家在冻结该案例独立判断后立即体验。完整学生沙盒允许持续聊天，不设置固定时长或轮次上限；约 1 小时给出非阻断休息提醒，超过 3 小时只生成最少必要的教师关注提示。它是情绪表达与梳理型 chatbot，不是心理咨询、诊断、治疗或危机服务。

成人评价开始时先完成 3 个不调用 Qwen、语音、自由文本或电话的隔离合成学生端微任务：选择固定心情、发送固定合成表达、打开并使用呼吸工具。每项可记录成功或无法完成；服务端保存错误尝试次数、从开始到终态的经过时间，并在任务后立即封存一项 1–5 分实际易用性评价。该行为证据只用于成年教师／专家对隔离微任务的形成性判断，不能代表真实学生或完整学生端的实际易用性。
...
v5.3.0 是 **18 岁以上成人操作的合成学校沙盒**。它保留虚构学生账号、心情记录、持续 Qwen 对话、ASR/TTS 和模拟教师处置，供作者、教师与专家使用固定虚构情境检查完整产品流程。所有学校域记录必须标记为 `synthetic=1`；登录前必须确认已成年且只使用合成信息；疑似真实姓名、学校、班级、学号、电话、邮箱、地址或账号信息会在保存或发送给文本模型前被拒绝。沙盒记录不进入 `/evaluate` 的成人研究评价表，也不能作为学生效果或干预效果证据。
...
当前成人教师/专家评价按已批准方案开展；为保护双盲审稿，审批机构、编号、日期与研究者身份保存在受控研究档案中，不写入公开页面或匿名稿件。
...
面向中国中小学生设计的每日心情记录、多轮 AI 情绪表达与梳理及真人支持研究系统。当前 **v5.4.0** 仅开放给成人使用合成账号和虚构情境测试；v5.3.2 及此前版本均作为独立历史版本保留。当前沙盒不代表已获准向未成年人公开招募或投入常态化使用。
...
- 学生可以与明确标注为 AI 的 Qwen 心伴连续对话。对话不设固定时长或轮次上限；短时服务端频率限制只用于阻断脚本刷模型。约 1 小时提醒休息，超过 3 小时给教师独立的非危机关注提示。
- Qwen 文本、ASR 语音转写和 TTS 主动朗读均通过服务端调用；单段录音最多 2 分钟且不超过 8 MB，不使用声音克隆，不自动播放，音频只在请求内存中转写且不写入数据库或对象存储。
...
v1.0.0 至 v5.4.0 的固定说明页和 Release 均保留，新版本不覆盖历史版本。`docs/versions/manifest.json` 是版本选择页的数据源，`releases/` 保存各版本的发布说明。
