...
当前归档版本为 **v4.0.1**。它沿用 v4.0.0 的学生端与教师端界面，只修复 Qwen 北京地域动态 TTS 音频域名和当前 ASR 时长字段兼容，并完成受控环境中的 Qwen 文本、ASR 语音转写与 TTS 云端朗读三链路联调。公开归档页仍为静态说明，不连接真实模型或数据库；联调通过不表示已经获准开展真实未成年人试点。
...
供应商必须返回 `usage.seconds`，或当前兼容响应中的
`usage.prompt_tokens_details.audio_tokens`（按官方 25 音频 Token/秒复核），
否则接口失败关闭。本原型另有单实例内存令牌桶
