...
面向中国中小学生的每日心情记录、短时 AI 陪伴与真人支持研究系统。当前 **v5.2.0** 是“成人评估公开演示候选版”；v5.0.0 的受控学校账号、D1 持久化和 Qwen API 链路仍作为独立历史版本保留。当前公开演示不代表已获准向未成年人公开招募或投入常态化使用。
...
v1.0.0 至 v4.0.1 的固定说明页和 Release 均保留；v5.0.0 新建归档，不覆盖历史版本。`docs/versions/manifest.json` 是版本选择页的数据源，`releases/` 保存各版本的发布说明。
