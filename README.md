...
面向中国中小学生设计的每日心情记录、短时 AI 陪伴与真人支持研究系统。当前 **v5.3.1** 仅开放给成人使用合成账号和虚构情境测试；v5.3.0、v5.2.0 的成人教师/专家评价版及 v5.0.0 的受控学校账号原型均作为独立历史版本保留。当前沙盒不代表已获准向未成年人公开招募或投入常态化使用。
...
v1.0.0 至 v5.3.1 的固定说明页和 Release 均保留，新版本不覆盖历史版本。`docs/versions/manifest.json` 是版本选择页的数据源，`releases/` 保存各版本的发布说明。
